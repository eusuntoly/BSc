...
cauta([H|_], H, 1).
cauta([_|T], H, Contor) :-
	cauta(T, H, C), Contor is C + 1, !.

%%	VERIFICA PATRATE
verificaPatratele(L1, L2, L3, Start, Stop, X) :-
	Start > Stop.
verificaPatratele([_|T1], [_|T2], [_|T3], Start, Stop, X) :-
	Start > 1,
	B is Start - 1,
	E is Stop - 1,
	verificaPatratele(T1, T2, T3, B, E, X).
verificaPatratele([H1|T1], [H2|T2], [H3|T3], 1, Stop, X) :-
	H1 \== X,
	H2 \== X,
	H3 \== X,
	E is Stop - 1,
	verificaPatratele(T1, T2, T3, 1, E, X).

%% VERIFICA LINII
extrageLinie([H|_], 1, H).
extrageLinie([_|T], C, H) :-
	C > 1, C1 is C - 1, extrageLinie(T, C1, H).

extrageElemente([], []).
extrageElemente([n|T], T1) :-
	extrageElemente(T, T1).
extrageElemente([H|T], [H|T1]) :-
	H \== n, extrageElemente(T, T1).


%% ADAUGA
adpos(1, X, [_|T], [X|T]).
adpos(P, X, [H|T], [H|T1]) :-
	P1 is P - 1, adpos(P1, X, T, T1).
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	Afisare
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
afis(Sudoku):- reverse(Sudoku,Ukodus),nl, afisare(Ukodus, 0).
