...
% 1
search([], _, [], _) :- !.
search([E|T], C, [C1|L], E) :- C1 is C + 1, search(T, C1, L, E), !.
search([E1|T], C, L, E) :- C1 is C + 1, search(T, C1, L, E), !.

% 2
poz(_, [], _, []) :- !.
poz([H|T], [C|T1], C, [H|L]) :- C1 is C + 1, poz(T, T1, C1, L), !.
poz([_|T], T1, C, L) :- C1 is C + 1, poz(T, T1, C1, L), !.
...
lung([], 0).
lung([H|T], Lung) :- lung(, Lung1), Lung is Lung + 1.
sum([], 0).
sum([H|T], S) :- sum(T, S1), S is S1 + H.
calculeazaMedie(L, M) :- sum(L, S), lung(L, Lung), Lung > 0, M is S / lung.

% 1
medie(N, P, M) :- studenti(N, P, F, I), findall(Note, catalog(_, F, Note, I), L), calculeazaMedie(L, M).
